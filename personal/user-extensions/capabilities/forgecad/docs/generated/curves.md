...
- `samples?: number` — Optional sample count when [`path`](sketch.md#path) is a path-like object.
...
The path's first coordinate becomes X and its second coordinate becomes Z. Use this for left/right silhouette rails authored with [`path()`](sketch.md#path) or [`constrainedSketch()`](sketch.md#constrainedsketch).

#### `Loft.pathOnYz(path: LoftPath2D, x?: number): Vec3[]` — Place a 2D guide path onto the YZ plane.

The path's first coordinate becomes Y and its second coordinate becomes Z. Use this for front/back crown rails authored with [`path()`](sketch.md#path) or [`constrainedSketch()`](sketch.md#constrainedsketch).
...
`EndCondition` — defined in [sketch](sketch.md).
...
`SheetEdge` — defined in [core](core.md).
...
`SheetPathAlongOptions` — defined in [core](core.md).
...
`SheetBoundaryPathSpan` — defined in [core](core.md).

`SheetBoundaryPathOptions` — defined in [core](core.md).
...
`SheetFrameOptions` — defined in [core](core.md).
...
When called after the initial [`path()`](sketch.md#path), this establishes the start of the outline. Calling `moveTo` again mid-path starts a new sub-path (hole in `close()`, separate segment for [`stroke()`](sketch.md#stroke)).
...
#### `close(): Sketch` — Close the path and return a filled [`Sketch`](sketch.md#sketch).
...
Edge labels (assigned with `.label('name')`) are transferred to the resulting sketch and propagate through `extrude()`, `revolve()`, `loft()`, and `sweep()` into named faces on the resulting [`Shape`](core.md#shape).
...
`ShapeMaterialProps` — defined in [core](core.md).
