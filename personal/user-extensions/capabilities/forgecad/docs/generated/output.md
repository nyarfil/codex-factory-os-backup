...
Point arguments accept 2D tuples `[x, y]`, 3D tuples `[x, y, z]`, or [`Point2D`](sketch.md#point2d) objects (Z is treated as 0 for 2D inputs).

Entity arguments: pass a single [`Line2D`](sketch.md#line2d) (from a constrained sketch) or an `EdgeRef` (from `shape.edge('left')`) as the first argument to dimension along that entity directly — no manual endpoint extraction needed.
...
[`Line2D`](sketch.md#line2d) / `EdgeRef` entity (then pass `opts` as the second argument)
