...
Use [`text2d()`](sketch.md#text2d) only when the letters should become manufactured geometry, such as raised lettering, engraved serial numbers, or exported nameplates.
...
- [`Shape`](core.md#shape) — highlight an entire 3D shape
...
Pass `{ labels: true }` with a [`Shape`](core.md#shape) to annotate every user-authored labeled face with its name (the face-label debugging view).
...
Mock objects appear in the viewport and inspection analysis when you run a file directly, but are excluded when the file is imported via [`require()`](core.md#require). This lets you model the surrounding context — walls, bolts, mating parts — without polluting the module's exports.
