...
Raw `SdfShape` values become mesh-backed [`Shape`](core.md#shape)s. Plain objects and arrays preserve their renderable children as a [`ShapeGroup`](core.md#shapegroup) when more than one leaf is found. Non-renderable metadata is ignored for materialization and remains available to callers through normal [`require()`](core.md#require) return values.
