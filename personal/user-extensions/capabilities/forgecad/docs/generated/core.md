...
- Pass a tracked `EdgeRef` from `shape.edge('vert-br')` (vertical edges of `box()` / [`Rectangle2D`](sketch.md#rectangle2d) extrusions) — this takes the **exact** compiler-owned path, not the mesh-approximate one
...
Default orientation places the first vertex at the top (90 degrees), matching the convention used by [`ngon()`](sketch.md#ngon).
...
When importing a `.forge.js` file, return values are passed through exactly as the script returns them, except for unsolved assemblies: a returned [`Assembly`](assembly.md#assembly) is wrapped as an [`ImportedAssembly`](assembly.md#importedassembly), preserving `solve(state)` and `mergeInto()` across file boundaries. A returned [`SolvedAssembly`](assembly.md#solvedassembly) stays a [`SolvedAssembly`](assembly.md#solvedassembly).
...
[`ImportedAssembly`](assembly.md#importedassembly) exposes default-pose helpers such as `getPart()`, `collisionReport()`, and `minClearance()`. Use `solve(state)` first when inspecting a non-default pose.
...
Anchors are metadata only: they do not change geometry. The editor uses them to show clickable parameter pins and manual-editing sheets in the 3D viewport. Use point anchors for scalar/text/list parameters and sheet anchors for `path2d`, [`spline2d`](curves.md#spline2d), and `placement2d` editors.
