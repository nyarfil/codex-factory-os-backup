...
`sketch` must be an **unplaced** compile-covered 2D profile (e.g. the result of [`circle2d()`](sketch.md#circle2d), [`rect()`](sketch.md#rect), [`roundedRect()`](sketch.md#roundedrect)). Passing an already-placed sketch (one that has had `.onFace(...)` called on it) will throw.
...
Applies all flanges (bent up at their configured angles) and all registered cutouts, then returns the resulting [`Shape`](core.md#shape). The shape is compiler-owned and exact-exportable (STEP, IGES, etc.).
