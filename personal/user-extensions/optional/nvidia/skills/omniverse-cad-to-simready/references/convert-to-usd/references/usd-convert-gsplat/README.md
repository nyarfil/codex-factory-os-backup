...
- Upstream skill: `https://github.com/NVIDIA-Omniverse/usd-convert-gsplat/blob/main/.agents/skills/usd-convert-gsplat/GUIDE.md`
...
Access note: Browser or raw-file fetches of the upstream skill URL can fail when the repo requires GitHub credentials. If that happens, use an authenticated local clone of `https://github.com/NVIDIA-Omniverse/usd-convert-gsplat` and read `.agents/skills/usd-convert-gsplat/GUIDE.md` from that checkout. Prefer `$PHYSICAL_AI_SKILL_HUB_UPSTREAM_ROOT/usd-convert-gsplat` or `$HOME/.physical-ai-skill-hub/upstreams/usd-convert-gsplat`.
