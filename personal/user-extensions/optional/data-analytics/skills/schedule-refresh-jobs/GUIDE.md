...
Use this skill for dashboard `schedule-refresh` actions and requests to schedule dashboard or report updates. Create and manage the automation in a cloud task so it runs independently of the user's computer. One-time refreshes and scheduled runs belong to [$build-dashboard](../build-dashboard/GUIDE.md) or [$build-report](../build-report/GUIDE.md). Do not turn document updates, change alerts, or summary sharing into refresh jobs.
...
Use [$build-dashboard](../build-dashboard/GUIDE.md) or [$build-report](../build-report/GUIDE.md), as appropriate, and the [shared refresh workflow](../../shared/data-app.md#refresh-a-published-dashboard-or-report). Save a prompt like this with the exact Site URL and known project/app IDs:
