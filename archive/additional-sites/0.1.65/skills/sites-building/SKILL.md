...
4. As soon as implementation is complete, perform the static-file checks in the selected **Project setup** reference for plain static assets; otherwise run `node <plugin-root>/scripts/build-site.mjs` if the output is missing or the source has changed since the last successful build. Keep any retained development server alive. Fix build or asset errors before hosting. Run lint separately only if the user asks.
5. Follow the shared preview rules below.
6. Continue to `sites-hosting` to publish, following its deployment rules. Avoid an unnecessary polish pass before publishing.
...
- After the complete implementation, perform the static-file checks in the selected **Project setup** reference for plain static assets; otherwise run `node <plugin-root>/scripts/build-site.mjs` if the output is missing or the source has changed since the last successful build. If a D1 schema changed, generate and inspect its migration. Fix real failures before hosting.
...
After creating or editing a site, use `sites-hosting` to publish and return the deployed Sites URL as the primary deliverable. If the user explicitly requested local-only work, saving without deploying, or no publishing, hand off the completed result without deploying. Follow the hosting skill's deployment rules. Do not include file paths, commands, or validation jargon unless the user asks. Keep any development server running until hosting finishes, then stop it during final teardown using the environment's preview controls.
