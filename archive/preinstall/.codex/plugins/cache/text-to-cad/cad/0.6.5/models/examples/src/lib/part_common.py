...
# `mode=None` rather than `mode=bd.Mode.ADD`: a default argument is evaluated at
# def time, so naming a kernel enum there would import build123d the moment a
# model imports this module — exactly the eager import the `bd.` idiom exists to
# avoid. The default is resolved on call instead.
def cylinder_axis_x(
    radius: float, length: float, center: tuple[float, float, float], *, mode=None
):
    return bd.Cylinder(
        radius=radius,
        height=length,
...
def label(part, name: str, color: bd.Color | None = None):
    part.label = name
