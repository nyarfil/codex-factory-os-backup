...
def search(root,end,side,fr,n=9000):
 ...
 for k in range(n):
  c1=root+[side*rng.uniform(1.5,5),rng.uniform(-1,5),rng.uniform(-10,-2)]
...
file=SRC/'thumb_fixed_anchors.json';data=json.loads(file.read_text());end=np.array(data['-1']['anchor'])+[1.88,0,-.85]
best=search([-2.27,-24,-7.36],end,-1,'thumb',16000);data['-1']['arms'][0]=best;file.write_text(json.dumps(data,indent=2)+'\n');print(best,flush=True)
