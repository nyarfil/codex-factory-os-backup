...
ring=make_retaining_ring(opening_half_angle=40);candidates=[(angle,base*bd.Rot(0,0,angle)*ring) for angle in range(0,360,5)]
...
report={'sample_count':225,'checks':checks,'rows':[dict(angle=a,minimum_gap=mins[a],worst=worst.get(a)) for a,_ in candidates]};(ROOT/'cmc_dorsal_ring_rotation_wide_probe.json').write_text(json.dumps(report,indent=2))
