...
def packet(previous,f,y):
 ...
 result={'flex':f,'yaw':y,'outlet_y':16,'rows':rows};out.append(result);(root/'thumb_cmc_top_corners_candidates.json').write_text(json.dumps(out,indent=2));print('POSE',f,y,round(time.time()-t,2),flush=True);return result
start=json.loads((root/'thumb_cmc_top_inner_first.json').read_text())[0]
for sequence in ((-5.,-10.,-15.,-20.,-25.),(5.,10.,15.,20.,25.,30.,35.,40.,45.)):
 previous=start
