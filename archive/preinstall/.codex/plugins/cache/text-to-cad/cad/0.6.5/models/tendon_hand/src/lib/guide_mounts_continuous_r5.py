...
def _rail_saddle(host,x,y,z,label,rail_width=1.45):
    ...
    """
    outer=bd.Pos(x,y,z)*bd.Box(rail_width+.80,1.10,2.4)
    outer=bd.fillet(outer.edges(),.10)
    shell=_cut(outer,host)
...
def make_phalanx_comb(length,width,station,lanes,label='reaction_comb'):
    ...
    # Exact local host patch: every comb body stays within station +/- .66.
    host=_common(host,(bd.Pos(0,station,0)*bd.Box(width+4,2.0,20)))
    ...
    for sign in (-1,1):
        x=sign*railx
        section=_common(host,(bd.Pos(sign*width/2,y,-3.5)*bd.Box(width,.02,4)))
        ...
        lower,upper,bolt=_rail_saddle(host,x,y,z,label,bb.size.X)
