...
shapes={'main':read_step(B/'imported/palm_frame_integration.step'),'little':read_step(B/'palm_little_review.step')}
...
for j,sample in enumerate(M['rows']):
 ...
 report={'scope':'All DIP± full path groups in every225 static packet versus final main/little, conservative primitive screen then exact native distances. Immutable PIP/CMC bearing contacts are separately owned by root.','completed_poses':j+1,'native_checks':native,'near':near,'unique_path_body_pairs':len(cache),'complete':j==224}
 (V/'palm_dip_225_scan.json').write_text(json.dumps(report,indent=2))
