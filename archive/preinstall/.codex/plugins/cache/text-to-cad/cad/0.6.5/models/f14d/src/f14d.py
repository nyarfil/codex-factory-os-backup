...
composed here by CALLING them — which is also how the embedded animation moves
...
the ``ANIMATION_JS`` literal below in the same commit. A system that fails to build fails the
...
staged teardown is choreography, which lives whole in ``ANIMATION_JS`` below.
"""
...
# ANIMATION_JS must be renumbered in the same commit.
SYSTEMS = [
    airframe,
    cockpit,
    wings,
...
ANIMATION_JS = r'''// Choreography for the F-14D Super Tomcat (copied into the sidecar at build;
...
@step(out="../STEP/f14d.step", animation=ANIMATION_JS)
def f14d():
