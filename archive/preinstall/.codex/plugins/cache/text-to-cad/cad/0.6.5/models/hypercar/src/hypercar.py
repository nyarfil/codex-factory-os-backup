...
the embedded ``ANIMATION_JS`` choreography targets:
...
# ANIMATION_JS below. Each entry is a sibling MODEL (src/<system>.py): calling it
...
ANIMATION_JS = r'''// Mid-engine hypercar -- choreography.
//
...
@step(out="../STEP/hypercar.step", kinematics=KINEMATICS, animation=ANIMATION_JS)
def hypercar():
