...
for i,s in enumerate(manifest['rows']):
 ...
 for part in parts:
  ...
  cache.add(key);placed=bd.Compound.cast(part.wrapped.Moved(matrix_location(m).wrapped));bb=placed.bounding_box(optimal=False)
  if any(hb.max.to_tuple()[a]<bb.min.to_tuple()[a] or bb.max.to_tuple()[a]<hb.min.to_tuple()[a] for a in range(3)):continue
  d=host.distance_to(placed)
  if d>.001:continue
  c=host&placed;volume=sum(x.volume for x in c.solids()) if c else 0
  row={'sample':s['label'],'part':part.label,'volume':volume,'gap':d};rows.append(row);print(row,flush=True)
 report={'complete':i==224,'sample_count':i+1,'unique_body_poses':len(cache),'rows':rows,'pass':all(r['volume']<1e-7 for r in rows)};(V/f'palm_{which}_final_comb_225_gate.json').write_text(json.dumps(report,indent=2))
