...
def frame(name):
 if '_palm_bank_' in name:return 'wrist_flexion'
...
if __name__=='__main__':
 ...
 parts=[p for f in files for p in leaves(read_step(f))];frames=[frame(p.label) for p in parts];bounds=np.array([corners(p) for p in parts]);manifest=json.loads((HERE/'static_route_packet_manifest.json').read_text())
