...
start=next(p for p in json.loads((root/'thumb_cmc_dorsal_bank_fourspan.json').read_text()) if p['flex']==55)
...
def packet(previous,f,y):
 ...
 for row in previous['rows']:
  ...
  if abs(row['lane'])!=3 and (f,y) in oldbank:seeds.insert(0,next(r['params'] for r in oldbank[(f,y)]['rows'] if r['lane']==row['lane']))
 ...
 result={'flex':f,'yaw':y,'outlet_y':16,'rows':rows};out.append(result);(root/'thumb_cmc_final_axes_candidates.json').write_text(json.dumps(out,indent=2));print('POSE',f,y,round(time.time()-t,2),flush=True);return result
previous=start
for f in range(50,-16,-5):previous=packet(previous,float(f),0.)
