"""Native solid separation and intersection gate with exact proof reuse."""
...
def audit(bodies,out,cache=None,changed_names=None,pose=None,on_progress=None):
    ...
    active=[];cache={} if cache is None else cache;checks=0;reused=0;separated=0;failures=[];rows=[]
    for number,a in enumerate(ordered):
        ...
        for b in active:
            ...
            if key in cache:volume=cache[key];reused+=1
                ...
                gap=separation(a[0].shape.wrapped,b[0].shape.wrapped)
                if gap is not None:
                    volume=0.;separated+=1
                else:
                    ...
                    props=GProp_GProps();BRepGProp.VolumeProperties_s(op.Shape(),props);volume=float(props.Mass());checks+=1
                cache[key]=volume;cache[geometric_key]=volume
            if volume>1e-7:
        ...
        if number%25==0 or number+1==len(ordered):
            report={'body_count':len(bodies),'processed':number+1,'exact_booleans':checks,'native_distance_separations':separated,'congruent_proof_reuse':reused,'collisions':failures,'complete':number+1==len(ordered),'pass':number+1==len(ordered) and not failures}
            ...
            if on_progress is not None and (number%100==0 or number+1==len(ordered)):on_progress(cache)
    return report
