...
def main():
    ...
    for name in CHAIN:
        ...
        path=HERE.parents[0]/'STEP/thumb_reaction_arm_clearance_r5.step' if name==CHAIN[0] else sources[rev['step_sha256']]
    ...
    report['pass']=report.pop('pass_');(HERE/'thumb_arm_r5_attachment_gate.json').write_text(json.dumps(report,indent=2)+'\n');assert report['pass']
