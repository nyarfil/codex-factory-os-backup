...
parts=leaves(read_step(ROOT/'STEP/phalanx_comb_clearance_review.step'));rows=[]
for f in FINGERS:
 for role,i,station in [('mcp_outlet',0,12.25),('pip_inlet',0,f.lengths[0]-12.25),('pip_outlet',1,10 if f.name=='little' else 12.25)]:
  ...
  for j,p in enumerate(allp):
   for k in range(j):
    ...
    def point_contact(a,b):
     ...
     return False
    d=0.025 if point_contact(p,allp[k]) or point_contact(allp[k],p) else p.distance_to(allp[k])
  ...
  result={'pass':all(not r['unattached'] and not r['host_interference'] for r in rows),'body_count':sum(r['bodies'] for r in rows),'clamp_thread_radial_fit_mm':.02,'rows':rows};Path(__file__).with_name('phalanx_comb_clearance_attachment_report.json').write_text(json.dumps(result,indent=2)+'\n')
