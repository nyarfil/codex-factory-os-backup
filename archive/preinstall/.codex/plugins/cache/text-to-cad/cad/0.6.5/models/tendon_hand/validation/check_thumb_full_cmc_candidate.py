...
import lib.thumb_routing as thumb_routing
from lib.thumb_cmc_transport import thumb_cmc_packet,cmc_inlet_contract
_candidate_atlas=json.loads(Path(sys.argv[1]).read_text())
def candidate_contract():
 rows=cmc_inlet_contract()
 for row in rows:
  if 'mcp_flexion' in row['tendon']:row['outlet'][1]=16.
 return rows
thumb_routing.cmc_inlet_contract=candidate_contract
def thumb_routes(pose=None):
 pose=dict(pose or {})
 packet=thumb_cmc_packet(pose.get('thumb_cmc_flexion',0),pose.get('thumb_cmc_abduction',0),atlas_override=_candidate_atlas,outlet_y=16.)
 return thumb_routing.thumb_routes(pose,cmc_packet=packet)
...
if __name__=='__main__':
    hardware=thumb_hardware();cache={};report={'scope':[n for n,f,p in hardware],'rows':[],'pass':False};out=Path(__file__).with_name('thumb_full_outlet16_report.json')
    samples=[(f"cmc_{p['flex']}_{p['yaw']}",p.get('whole_pose',{'thumb_cmc_flexion':p['flex'],'thumb_cmc_abduction':p['yaw']})) for p in _candidate_atlas]
    out=Path(sys.argv[1]).with_name(Path(sys.argv[1]).stem+'_full_thumb_audit.json')
        ...
        for route in routes:
            for group in route['groups']:
                ...
                for name,frame,part,original in solids:
                    ...
                    distance=cache[key]
                    if tested%50==0:print("DISTANCES",label,tested,flush=True)
