...
Use a chart when it clarifies a comparison, trend, distribution, or relationship; otherwise prefer a compact table or summary. Give each chart a distinct, data-supported takeaway. Create native, editable workbook charts, not static images.
...
These are defaults, not restrictions; choose what makes the takeaway clearest:
...
- Add axis titles only when meaning or units are unclear. Label values selectively when exact numbers matter or axes are hard to read; avoid labeling every point on dense lines. Prefer direct labels for a few series; use a legend when needed without crowding the plot.
- Use color consistently for meaning; keep single-series charts one color unless highlighting meaningful differences. Avoid unnecessary gradients, heavy borders, excessive gridlines, and decorative effects unless requested; retain useful reference lines.
