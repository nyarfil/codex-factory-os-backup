...
- Include sanity checks: for example, if you calculate a total patient count from different categories, include a check that the sum of categories equals the total from the raw data. Independently verify critical calculations against an appropriate authoritative source or qualified review and state when that verification is unavailable. A formula comment is not verification; use only source-supported thresholds and definitions and do not invent clinical advice or change source protocols.
